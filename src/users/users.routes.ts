...
router.post('/login', passport.authenticate('login', { failureMessage: true, failureRedirect: '/api/users/loginError' }), (req: Request, res: Response) => {
    res.status(200).json({
        data: 'ok'
    });
});
...
router.post('/loginError', (req, res) => {
    res.json({
