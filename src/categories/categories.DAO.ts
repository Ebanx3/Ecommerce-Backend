...
export const getCategoryById = async (req: Request, res: Response) => {
    ...
    category == 'Category not found, invalid Id' ? res.status(400) : res.status(200)
    res.json({
        data: category
    })
};
...
export const createCategory = async (req: Request, res: Response) => {
    ...
    if(newCategory == 'Name field is necessary'){
        res.status(400).json({
            data: newCategory
        })
        return
...
export const deleteCategory = async (req: Request, res: Response) => {
    ...
    deletedCat == 'Category not found, invalid Id' ? res.status(400) : res.status(200);
    res.json({
        data: deletedCat
    })
};